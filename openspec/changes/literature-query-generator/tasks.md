...
- [ ] 3.1 Implement `init_db()`: Copy seed DB, create `queries` table with NOT NULL `summary_id` and FOREIGN KEY ... ON DELETE CASCADE
...
- [ ] 5.1 Document the direct delegation step: call `paper-summarizer` first for all files, check if it aborts, and extract output
